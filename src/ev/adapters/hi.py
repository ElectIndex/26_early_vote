...
def build(text: str, cycle: int, *, filename_date: date | None = None) -> FetchResult:
    ...
    if totals is not None and len(counties) == len(COUNTY_NAMES):
        ...
        # PARTIAL COVERAGE, handled exactly as tx.py handles a short Texas
        # report: county rows are published and no statewide row is, because a
        # total over three of Hawaii's four counties looks precisely like a
        # Hawaii turnout figure and is not one.
        log.warning(
            "HI: report covers %d of %d counties; publishing counties only",
            len(counties), len(COUNTY_NAMES),
        )
